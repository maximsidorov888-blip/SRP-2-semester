...
@njit
def solve_nqueens(N, use_hysteresis=True, max_era=3000, step=0.3, alpha_learn_rate=0.01, beta_learn_rate=0.01,
    ...
    for era in range(max_era):
        ...
        if cooldown_counter > 0:
            ...
            '''if stagnation_counter >= 50 and use_hysteresis:
                cooldown_counter = 20
...
if __name__ == "__main__":
    ...
    # Final test: HHNN (with "best" parameters found in Optuna search)
    HHNN_Plateau_success = 0
    for _ in range(5000):
        sol, _ = solve_nqueens(50, use_hysteresis=True, step=0.3, alpha_learn_rate=0.0039, beta_learn_rate=0.0026)
        if sol is not None:
            HHNN_Plateau_success += 1
    print(f"Success rate of HHNN (5000 tests): {HHNN_Plateau_success / 5000 * 100:.3f}%")
    HHNN_Plateau_time = time.time()
    print(f"Total execution time: {HHNN_Plateau_time - start_time:.2f} s.")
   

    # Final test: HNN (Basic)
    HNN_success = 0
    for _ in range(5000):
        sol, _ = solve_nqueens(50, use_hysteresis=False, step=0.3)
        if sol is not None:
            HNN_success += 1
    print(f"Success rate of HNN (5000 tests): {HNN_success / 5000 * 100:.3f}%")
    HNN_time = time.time()
    print(f"Total execution time: {HNN_time - HHNN_Plateau_time:.2f} s")
